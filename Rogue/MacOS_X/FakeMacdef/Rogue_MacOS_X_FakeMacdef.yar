...
rule Rogue_MacOS_X_FakeMacdef_2{
	...
	strings :
		...
		$a_03_2 = {89 54 24 04 89 04 24 e8 ?? ?? ?? ?? 83 f8 01 19 f6 83 e6 02 46 8b 83 ?? ?? ?? ?? 89 44 24 04 8b 83 ?? ?? ?? ?? 89 04 24 e8 } //2
		$a_03_3 = {8b 7d 08 0f b6 75 10 c6 87 ?? ?? 00 00 00 c7 44 24 08 00 00 80 3e 8b 83 ?? ?? ?? ?? 89 44 24 04 89 3c 24 e8 } //2
	condition:
...
rule Rogue_MacOS_X_FakeMacdef_3{
	...
	strings :
		...
		$a_03_2 = {89 54 24 04 89 04 24 e8 ?? ?? ?? ?? 83 f8 01 19 db 83 e3 02 43 a1 ?? ?? ?? ?? 89 44 24 04 a1 ?? ?? ?? ?? 89 04 24 e8 } //2
		$a_03_3 = {8b 75 08 0f b6 5d 10 (c6 86 ?? ?? 00 00 00 c6|46 ?? 00 c7 44 24) 08 00 00 80 3e a1 ?? ?? ?? ?? 89 44 24 04 89 34 24 e8 } //2
	condition:
		((#a_00_0  & 1)*2+(#a_00_1  & 1)*2+(#a_03_2  & 1)*2+(#a_03_3  & 1)*2) >=4
