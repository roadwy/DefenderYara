...
rule Rogue_Win32_FakeRemoc{
	...
	strings :
		$a_03_0 = {41 6e 74 69 4d 61 6c 77 61 72 65 4d 61 73 74 65 72 ?? ?? ?? ?? ?? ?? ?? ?? 2d ?? ?? ?? ?? 2d ?? ?? ?? ?? 2d ?? ?? ?? ?? 2d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 00 } //1
		$a_02_1 = {54 6f 74 61 6c 53 63 61 6e 43 6f 75 6e 74 [0-06] 49 6e 66 65 63 74 69 6f 6e 43 6f 75 6e 74 [0-06] 49 73 50 61 69 64 } //1
	condition:
...
rule Rogue_Win32_FakeRemoc_3{
	...
	strings :
		...
		$a_03_1 = {be 1f 00 02 00 56 57 ff 75 e0 8d 4d e8 e8 ?? ?? ?? ?? 85 c0 74 32 68 19 00 02 00 } //1
...
rule Rogue_Win32_FakeRemoc_5{
	...
	strings :
		$a_03_0 = {41 6e 74 69 53 70 79 77 61 72 65 4d 61 73 74 65 72 ?? ?? ?? ?? ?? ?? ?? ?? 2d ?? ?? ?? ?? 2d ?? ?? ?? ?? 2d ?? ?? ?? ?? 2d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 00 } //10
