...
rule Rogue_Win32_FakePlus{
	...
	strings :
		...
		$a_03_2 = {54 6a 06 68 ?? ?? ?? ?? a1 ?? ?? ?? ?? 50 6a ff e8 ?? ?? ?? ?? c6 05 ?? ?? ?? ?? 68 c7 05 ?? ?? ?? ?? ?? ?? ?? ?? c6 05 ?? ?? ?? ?? c3 54 6a 06 68 ?? ?? ?? ?? a1 ?? ?? ?? ?? 50 6a ff e8 } //5
	condition:
...
rule Rogue_Win32_FakePlus_3{
	...
	strings :
		$a_03_0 = {51 68 e2 00 00 00 57 56 ff 15 ?? ?? ?? ?? 85 c0 74 19 81 7c 24 08 e2 00 00 00 75 0f } //2
		$a_03_1 = {30 1c 01 42 3b d7 7c 02 33 d2 41 81 f9 ?? ?? 00 00 7c } //2
...
rule Rogue_Win32_FakePlus_4{
	...
	strings :
		...
		$a_03_14 = {6a 01 6a 00 6a 02 6a 00 6a ff e8 ?? ?? ?? ?? 8b d8 e8 ?? ?? ?? ?? 3d b7 00 00 00 75 1b 68 10 00 04 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 6a 00 e8 } //6
