...
rule Trojan_Win32_Kovter_E{
	...
	strings :
		...
		$a_03_1 = {50 6a 00 6a 00 6a 24 6a 00 ff d6 50 e8 ?? ?? ?? ?? e8 ?? ?? ?? ?? e8 ?? ?? ?? ?? 85 c0 74 0a 8b c3 } //1
		$a_03_2 = {6a 00 6a 02 6a 02 6a 00 6a 00 68 00 00 00 40 8b 45 fc e8 ?? ?? ?? ?? 8b d8 53 e8 ?? ?? ?? ?? 50 e8 ?? ?? ?? ?? 8d 85 ?? ?? ?? ?? 50 68 ?? ?? ?? ?? 53 e8 ?? ?? ?? ?? 53 e8 } //1
