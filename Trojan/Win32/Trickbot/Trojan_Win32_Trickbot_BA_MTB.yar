...
rule Trojan_Win32_Trickbot_BA_MTB{
	...
	strings :
		$a_02_0 = {03 c1 8b cb 99 f7 f9 8b 4d ?? 8b 75 ?? 0f be 0c 31 51 0f b6 04 3a 50 e8 ?? ?? ?? ?? 83 c4 10 88 06 46 ff 4d ?? 89 75 ?? 75 } //1
	condition:
		((#a_02_0  & 1)*1) >=1
...
rule Trojan_Win32_Trickbot_BA_MTB_2{
	...
	strings :
		...
		$a_02_1 = {6a 00 6a 00 ff 15 ?? ?? ?? ?? 8b 44 24 10 8d 0c 06 33 d2 6a 14 8b c6 5b f7 f3 8b 44 24 0c 8a 04 02 30 01 46 3b 74 24 14 75 } //1
	condition:
...
rule Trojan_Win32_Trickbot_BA_MTB_3{
	...
	strings :
		$a_02_0 = {2b c8 33 c0 8a 04 11 8d 3c 11 89 85 ?? ?? ?? ?? db 85 ?? ?? ?? ?? dc 65 ?? dc 15 ?? ?? ?? ?? df e0 f6 c4 01 74 ?? dc 05 ?? ?? ?? ?? 83 ec 08 dd 1c 24 ff 15 ?? ?? ?? ?? ff 15 ?? ?? ?? ?? 8b 8d ?? ?? ?? ?? 88 07 01 4d ?? e9 } //1
	condition:
		((#a_02_0  & 1)*1) >=1
