...
rule Trojan_Win32_Trickbot_RM_MTB{
	...
	strings :
		$a_02_0 = {99 f7 fb 8b 75 ?? 8b c1 8a 1c 31 80 c2 4f 32 da 47 88 1c 31 b9 05 00 00 00 99 f7 f9 89 7d ?? 85 d2 75 } //1
	condition:
		((#a_02_0  & 1)*1) >=1
...
rule Trojan_Win32_Trickbot_RM_MTB_2{
	...
	strings :
		$a_03_0 = {03 e8 8d 44 6d ?? 2b d0 a1 ?? ?? ?? ?? 8b d8 0f af d8 8b c1 03 c2 8a 14 03 8b 44 24 ?? 8a 18 32 da 8b 54 24 ?? 88 18 8b 44 24 ?? 40 3b c2 89 44 24 ?? 0f } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win32_Trickbot_RM_MTB_3{
	...
	strings :
		$a_03_0 = {33 db 8a 1c 02 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b d8 03 ?? ?? ?? ?? ?? 8b ?? ?? 8a 04 0a 32 c3 8b ?? ?? 8b 11 8b ?? ?? 88 04 11 e9 } //1
...
rule Trojan_Win32_Trickbot_RM_MTB_4{
	...
	strings :
		$a_02_0 = {ff 30 50 ff 75 ?? 53 6a 01 53 ff 75 ?? ff 55 ?? 85 c0 0f 95 c0 eb ?? 32 c0 } //1
		$a_02_1 = {59 33 c0 bf e0 73 48 00 39 75 ?? f3 ab aa 89 1d ?? ?? ?? ?? 0f 86 ?? ?? ?? ?? 80 7d ?? 00 0f 84 ?? ?? ?? ?? 8d 4d ?? 8a 11 84 d2 0f 84 } //1
...
rule Trojan_Win32_Trickbot_RM_MTB_5{
	...
	strings :
		$a_03_0 = {89 45 e8 c7 45 ?? 42 00 00 00 e8 ?? ?? ?? ?? 8b 4d ?? 85 c9 76 ?? 8b 45 ?? 8d a4 24 ?? ?? ?? ?? 8a 10 80 f2 63 80 c2 63 88 10 83 c0 01 83 e9 01 75 } //10
...
rule Trojan_Win32_Trickbot_RM_MTB_6{
	...
	strings :
		$a_03_0 = {33 d2 f7 35 ?? ?? ?? ?? a0 ?? ?? ?? ?? 8a 14 0a 02 d0 8b 44 24 ?? 8a 1c 28 32 da 88 1c 28 8b 44 24 ?? 45 3b e8 72 } //1
