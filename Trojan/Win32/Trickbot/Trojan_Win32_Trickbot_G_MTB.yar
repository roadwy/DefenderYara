...
rule Trojan_Win32_Trickbot_G_MTB{
	...
	strings :
		$a_02_0 = {43 00 3a 00 5c 00 50 00 72 00 6f 00 67 00 72 00 61 00 6d 00 44 00 61 00 74 00 61 00 5c 00 [0-30] 2e 00 65 00 78 00 65 00 00 00 } //1
...
rule Trojan_Win32_Trickbot_G_MTB_2{
	...
	strings :
		$a_02_0 = {03 d0 8b 45 ?? 0f b6 14 10 8b 45 ?? 0f b6 0c 08 33 ca 8b 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 8b 75 ?? 2b f0 03 f2 2b 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 03 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 8b 55 ?? 88 0c 32 e9 } //10
