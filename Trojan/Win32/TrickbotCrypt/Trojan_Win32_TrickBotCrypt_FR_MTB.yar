...
rule Trojan_Win32_TrickBotCrypt_FR_MTB{
	...
	strings :
		$a_03_0 = {0f b6 11 03 c2 33 d2 f7 35 ?? ?? ?? ?? 89 55 f0 8b 45 f0 03 05 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 0f af 0d ?? ?? ?? ?? 0f af 0d ?? ?? ?? ?? 03 c1 2b 05 ?? ?? ?? ?? 8b 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 2b c2 2b 05 ?? ?? ?? ?? 2b 05 ?? ?? ?? ?? 8b 4d 08 0f b6 14 01 8b 45 0c 03 45 f4 0f b6 08 33 ca 8b 55 0c 03 55 f4 88 0a e9 } //10
