...
rule Trojan_Win32_TrickBotCrypt_GG_MTB{
	...
	strings :
		$a_03_0 = {8b 55 f0 2b 15 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 0f af 0d ?? ?? ?? ?? 2b d1 2b 15 ?? ?? ?? ?? 03 15 ?? ?? ?? ?? 2b 15 ?? ?? ?? ?? 2b 15 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 0f af 0d ?? ?? ?? ?? 0f af 0d ?? ?? ?? ?? 2b d1 2b 15 ?? ?? ?? ?? 8b 4d 08 0f b6 14 11 8b 4d 0c 0f b6 04 01 33 c2 } //1
