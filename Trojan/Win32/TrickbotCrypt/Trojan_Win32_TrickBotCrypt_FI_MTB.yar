...
rule Trojan_Win32_TrickBotCrypt_FI_MTB{
	...
	strings :
		$a_03_0 = {2b d0 2b 15 ?? ?? ?? ?? 03 15 ?? ?? ?? ?? 2b 15 ?? ?? ?? ?? 03 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b d0 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b d0 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b d0 2b 15 ?? ?? ?? ?? 8b 45 08 0f b6 14 10 8b 45 0c 0f b6 0c 08 33 ca 8b 55 f4 } //1
