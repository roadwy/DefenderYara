...
rule Trojan_Win32_TrickBotCrypt_EV_MTB{
	...
	strings :
		$a_03_0 = {0f b6 14 0e 0f b6 04 0f 03 c2 33 d2 bb 05 18 00 00 f7 f3 a1 ?? ?? ?? ?? 03 55 e0 8d 14 42 03 55 dc 8b 45 f8 03 15 ?? ?? ?? ?? 40 03 15 ?? ?? ?? ?? 89 45 f8 8a 1c 0a 8b 55 fc 30 5c 02 ff } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win32_TrickBotCrypt_EV_MTB_2{
	...
	strings :
		$a_03_0 = {8b 4d 08 0f b6 04 01 8b 4d 0c 0f b6 14 11 33 d0 8b 45 f4 2b 05 ?? ?? ?? ?? 2b 05 ?? ?? ?? ?? 2b 05 ?? ?? ?? ?? 8b 4d 0c 88 14 01 e9 } //1
