...
rule Trojan_Win32_TrickBotCrypt_ED_MTB{
	...
	strings :
		$a_03_0 = {8b 54 24 14 03 d0 a0 ?? ?? ?? ?? f6 eb 2a c8 8a c1 b1 1f f6 e9 8b 4c 24 ?? 02 04 31 2a 05 ?? ?? ?? ?? 30 02 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win32_TrickBotCrypt_ED_MTB_2{
	...
	strings :
		$a_03_0 = {0f b6 14 0f 89 44 24 10 0f b6 04 0e 03 c2 33 d2 f7 35 ?? ?? ?? ?? b8 02 00 00 00 2b 05 ?? ?? ?? ?? 45 0f af c3 0f af c3 48 03 15 ?? ?? ?? ?? 0f af c3 03 c2 8a 14 08 8b 44 24 10 30 10 } //5
	condition:
		((#a_03_0  & 1)*5) >=5
