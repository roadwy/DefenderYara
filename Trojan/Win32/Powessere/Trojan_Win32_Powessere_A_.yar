...
rule Trojan_Win32_Powessere_A_{
	...
	strings :
		...
		$a_00_1 = {61 69 64 3d 25 73 26 62 75 69 6c 64 64 61 74 65 3d 25 73 26 69 64 3d 25 73 26 6f 73 3d 25 73 5f } //01 00  aid=%s&builddate=%s&id=%s&os=%s_
...
rule Trojan_Win32_Powessere_A__3{
	...
	strings :
		...
		$a_00_3 = {2d 00 6b 00 68 00 62 00 37 00 34 00 37 00 62 00 6a 00 67 00 33 00 32 00 34 00 79 00 75 00 } //01 00  -khb747bjg324yu
		$a_00_4 = {3c 63 6c 69 63 6b 75 72 6c 3e } //01 00  <clickurl>
		$a_00_5 = {7b 73 65 72 76 65 72 7d 2f 71 75 65 72 79 3f 76 65 72 73 69 6f 6e 3d } //01 00  {server}/query?version=
		$a_00_6 = {62 75 69 6c 64 64 61 74 65 3d 7b 62 75 69 6c 64 64 61 74 65 7d } //01 00  builddate={builddate}
		$a_00_7 = {77 74 3d 7b 74 68 72 65 61 64 73 7d } //01 00  wt={threads}
		$a_00_8 = {6c 72 3d 7b 6c 61 73 74 72 65 73 75 6c 74 7d } //01 00  lr={lastresult}
		$a_00_9 = {6c 73 3d 7b 6c 61 73 74 73 74 61 67 65 7d } //01 00  ls={laststage}
		$a_00_10 = {25 5b 5e 3b 5d 3b 25 5b 5e 3b 5d 3b 25 5b 5e 3b 5d 3b 25 2a 5b 5e 3b 5d 3b 25 2a 5b 5e 3b 5d 3b 25 75 } //01 00  %[^;];%[^;];%[^;];%*[^;];%*[^;];%u
		$a_00_11 = {25 2a 5b 5e 3b 5d 3b 25 2a 5b 5e 3b 5d 3b 25 2a 5b 5e 3b 5d 3b 25 2a 5b 5e 3b 5d 3b 25 75 } //02 00  %*[^;];%*[^;];%*[^;];%*[^;];%u
		$a_00_12 = {64 65 67 65 6e 65 72 61 74 69 76 65 2b 6a 6f 69 6e 74 2b 64 69 73 65 61 73 65 } //02 00  degenerative+joint+disease
		$a_00_13 = {61 6e 74 69 2b 61 67 69 6e 67 2b 73 6b 69 6e 2b 63 61 72 65 } //02 00  anti+aging+skin+care
		$a_00_14 = {6f 6e 6c 69 6e 65 2b 61 75 74 6f 2b 69 6e 73 75 72 61 6e 63 65 2b 71 75 6f 74 65 73 } //00 00  online+auto+insurance+quotes
	condition:
		any of ($a_*)
...
rule Trojan_Win32_Powessere_A__4{
	...
	strings :
		...
		$a_01_3 = {26 61 69 64 3d 25 73 26 62 75 69 6c 64 64 61 74 65 3d 25 73 26 69 64 3d 25 73 26 6f 73 3d 25 73 5f 25 73 } //01 00  &aid=%s&builddate=%s&id=%s&os=%s_%s
		$a_01_4 = {25 5b 5e 3b 5d 3b 25 5b 5e 3b 5d 3b 25 5b 5e 3b 5d 3b 25 5b 5e 3b 5d 3b 25 73 } //02 00  %[^;];%[^;];%[^;];%[^;];%s
		...
		$a_01_11 = {5c 41 64 6c 69 63 65 5c 52 6f 67 75 65 4b 69 6c 6c 65 72 } //f6 ff  \Adlice\RogueKiller
		$a_01_12 = {53 69 67 6e 61 74 75 72 65 42 6c 61 63 6b 6c 69 73 74 52 75 6c 65 50 6f 77 65 6c 69 6b 73 } //00 00  SignatureBlacklistRulePoweliks
	condition:
		any of ($a_*)
