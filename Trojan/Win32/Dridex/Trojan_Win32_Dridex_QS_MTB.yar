...
rule Trojan_Win32_Dridex_QS_MTB{
	...
	strings :
		$a_02_0 = {8d 84 0a 18 64 00 00 2b 45 e4 03 05 ?? ?? ?? ?? a3 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 81 e9 18 64 00 00 89 0d ?? ?? ?? ?? 8b 15 ?? ?? ?? ?? 03 55 e4 03 15 ?? ?? ?? ?? 89 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 03 45 e4 8b 0d ?? ?? ?? ?? 2b c8 89 0d ?? ?? ?? ?? ba c5 01 00 00 } //10
