...
rule Trojan_Win32_Dridex_PB_MTB_2{
	...
	strings :
		...
		$a_03_3 = {81 f1 80 8e b2 16 8b 94 24 ?? ?? ?? ?? 8b b4 24 ?? ?? ?? ?? 8b 7c 24 ?? 89 bc 24 ?? ?? ?? ?? 89 8c 24 ?? ?? ?? ?? 89 e1 [0-25] 8b 45 ?? 8b 8c 24 ?? ?? ?? ?? 01 c8 8a 9c 24 ?? ?? ?? ?? b7 cb 28 df 88 bc 24 ?? ?? ?? ?? 89 84 24 ?? ?? ?? ?? 8b 44 24 ?? 35 25 0a fc 52 } //2
	condition:
