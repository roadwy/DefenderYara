...
rule Trojan_Win32_Dridex_DA_MTB{
	...
	strings :
		$a_03_0 = {8b bc 2e f5 e4 ff ff 75 ?? 04 1e 02 c0 2a 05 ?? ?? ?? ?? 02 c1 66 0f b6 d0 66 6b d2 03 66 2b 15 ?? ?? ?? ?? 66 89 15 ?? ?? ?? ?? 81 c7 d4 e0 08 01 89 3d ?? ?? ?? ?? 89 bc 2e f5 e4 ff ff 8a 15 ?? ?? ?? ?? 66 8b 0d ?? ?? ?? ?? 8a c2 02 c1 83 c6 04 2c 02 81 fe 33 1c 00 00 0f 82 } //5
	condition:
		((#a_03_0  & 1)*5) >=5
...
rule Trojan_Win32_Dridex_DA_MTB_2{
	...
	strings :
		$a_02_0 = {8d 04 1f 03 f0 2b d6 83 ea 57 6b c2 56 89 15 ?? ?? ?? ?? 2b c8 6b c7 56 89 0d ?? ?? ?? ?? 2b c8 2b d1 89 0d ?? ?? ?? ?? 8d 42 f7 03 c7 a3 ?? ?? ?? ?? 33 c0 89 45 3c 8d 45 30 89 45 40 } //10
