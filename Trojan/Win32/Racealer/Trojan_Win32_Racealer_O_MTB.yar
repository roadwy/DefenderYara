...
rule Trojan_Win32_Racealer_O_MTB{
	...
	strings :
		$a_03_0 = {b8 36 23 01 00 01 45 fc [0-08] 03 55 08 8b 45 fc 03 45 08 8a 08 88 0a } //1
		...
		$a_03_4 = {83 e9 14 88 0d ?? ?? ?? ?? 0f be 15 ?? ?? ?? ?? 83 ea 14 88 15 ?? ?? ?? ?? 0f be 05 ?? ?? ?? ?? 83 e8 14 a2 ?? ?? ?? ?? 0f be 0d ?? ?? ?? ?? 83 e9 0a } //1
	condition:
