...
rule Trojan_Win32_Emotetcrypt_KH_MTB{
	...
	strings :
		$a_03_0 = {2b d0 2b 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 03 d0 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b d0 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 03 15 ?? ?? ?? ?? 03 c2 8b 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 03 55 14 0f b6 04 02 33 c8 8b 55 fc 2b 15 ?? ?? ?? ?? 2b 15 ?? ?? ?? ?? 2b 15 ?? ?? ?? ?? 03 15 } //1
