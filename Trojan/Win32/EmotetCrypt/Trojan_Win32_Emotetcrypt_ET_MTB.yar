...
rule Trojan_Win32_Emotetcrypt_ET_MTB{
	...
	strings :
		$a_03_0 = {8b 55 d8 2b f2 03 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 03 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 8b 45 dc 03 f0 8b 4d e0 03 f1 8b 55 e4 2b f2 03 35 ?? ?? ?? ?? 03 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 2b 35 ?? ?? ?? ?? 03 35 ?? ?? ?? ?? 03 35 ?? ?? ?? ?? 8b 45 0c 8b 4d e8 88 0c 30 e9 } //1
