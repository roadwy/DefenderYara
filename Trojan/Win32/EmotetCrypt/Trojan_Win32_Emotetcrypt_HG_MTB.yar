...
rule Trojan_Win32_Emotetcrypt_HG_MTB{
	...
	strings :
		$a_03_0 = {03 d0 8b 4d f0 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 8b 45 08 0f b6 0c 08 8b 45 0c 0f b6 14 10 33 d1 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 8b 4d f4 2b 0d ?? ?? ?? ?? 03 45 0c 88 14 08 } //1
