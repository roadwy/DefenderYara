...
rule Trojan_Win32_Emotetcrypt_IL_MTB{
	...
	strings :
		$a_03_0 = {03 c8 03 0d ?? ?? ?? ?? 03 0d ?? ?? ?? ?? a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b c8 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 03 45 08 0f b6 0c 08 8b 45 0c 0f b6 14 10 33 d1 a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 8b 4d f4 2b 0d ?? ?? ?? ?? 03 0d ?? ?? ?? ?? 03 45 0c 88 14 08 } //1
