...
rule Trojan_Win32_Emotetcrypt_GO_MTB{
	...
	strings :
		$a_03_0 = {2b d1 8b 4d ?? 0f b6 14 11 8b 4d ?? 0f b6 04 01 33 c2 8b 0d ?? ?? ?? ?? 0f af 0d ?? ?? ?? ?? 8b 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 8b 35 ?? ?? ?? ?? 0f af 35 ?? ?? ?? ?? 8b 3d ?? ?? ?? ?? 0f af 3d ?? ?? ?? ?? 0f af 3d ?? ?? ?? ?? 8b 5d ?? 03 1d ?? ?? ?? ?? 2b 1d ?? ?? ?? ?? 2b df } //1
