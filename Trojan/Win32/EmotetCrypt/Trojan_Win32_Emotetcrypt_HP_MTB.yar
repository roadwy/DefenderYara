...
rule Trojan_Win32_Emotetcrypt_HP_MTB{
	...
	strings :
		$a_03_0 = {2b ca 2b 0d ?? ?? ?? ?? 03 0d ?? ?? ?? ?? a1 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 0f af 05 ?? ?? ?? ?? 2b c8 8b 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? 03 ca 2b 0d ?? ?? ?? ?? 03 0d ?? ?? ?? ?? 2b 0d ?? ?? ?? ?? 03 0d ?? ?? ?? ?? 8b 45 f0 2b 05 ?? ?? ?? ?? 2b 05 ?? ?? ?? ?? 8b 55 08 0f b6 04 02 8b 55 0c 0f b6 0c 0a 33 c8 8b 15 ?? ?? ?? ?? 0f af 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 0f af 05 } //1
