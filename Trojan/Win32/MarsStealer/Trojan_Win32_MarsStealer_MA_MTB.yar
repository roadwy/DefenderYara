...
rule Trojan_Win32_MarsStealer_MA_MTB{
	...
	strings :
		$a_03_0 = {8b 44 24 14 8b 4c 24 10 c1 e8 05 03 44 24 28 33 cb c7 05 ?? ?? ?? ?? ?? ?? ?? ?? c7 05 ?? ?? ?? ?? ?? ?? ?? ?? 89 44 24 18 89 4c 24 10 8b 44 24 18 31 44 24 10 2b 74 24 10 81 c7 ?? ?? ?? ?? 4d } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win32_MarsStealer_MA_MTB_2{
	...
	strings :
		$a_03_0 = {83 c4 04 a3 ?? ?? ?? ?? 68 90 0a 19 00 8b ec 68 ?? ?? ?? ?? e8 [0-15] e8 ?? ?? ?? ?? 83 c4 04 a3 ?? ?? ?? ?? ?? 8b 44 24 04 05 1a 58 01 00 ff d0 } //10
