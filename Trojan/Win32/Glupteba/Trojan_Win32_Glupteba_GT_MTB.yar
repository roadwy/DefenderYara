...
rule Trojan_Win32_Glupteba_GT_MTB{
	...
	strings :
		$a_02_0 = {31 0f 29 c0 21 d8 81 c7 04 00 00 00 39 f7 75 eb 42 81 c2 ?? ?? ?? ?? c3 } //10
	condition:
		((#a_02_0  & 1)*10) >=10
...
rule Trojan_Win32_Glupteba_GT_MTB_2{
	...
	strings :
		$a_03_0 = {31 0e 01 da 81 c6 ?? ?? ?? ?? 81 c2 ?? ?? ?? ?? 29 d0 39 fe 75 e5 c3 } //10
		$a_03_1 = {31 0b 81 c2 ?? ?? ?? ?? 4e 81 c3 ?? ?? ?? ?? 4a bf ?? ?? ?? ?? 39 c3 75 e2 81 ee } //10
	condition:
