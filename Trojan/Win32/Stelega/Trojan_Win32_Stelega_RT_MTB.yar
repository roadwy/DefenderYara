...
rule Trojan_Win32_Stelega_RT_MTB{
	...
	strings :
		$a_03_0 = {03 c2 83 e0 03 0f b6 80 ?? ?? ?? ?? 30 82 ?? ?? ?? ?? 8b 85 ?? ?? ?? ?? 8d 80 ?? ?? ?? ?? 03 c2 83 e0 03 0f b6 80 ?? ?? ?? ?? 30 82 ?? ?? ?? ?? 83 c2 06 81 fa a0 bb 0d 00 0f } //2
	condition:
		((#a_03_0  & 1)*2) >=2
...
rule Trojan_Win32_Stelega_RT_MTB_3{
	...
	strings :
		$a_03_0 = {89 5d fc 31 4d ?? 8b 5d ?? c7 05 ?? ?? ?? ?? 01 00 00 00 01 1d ?? ?? ?? ?? ff 0d ?? ?? ?? ?? a1 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 89 08 5b 8b e5 5d } //1
		$a_03_1 = {81 c1 8a 10 00 00 8b 55 ?? 8b 02 2b c1 8b 4d ?? 89 01 8b 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 8d 4c 10 ?? 89 0d ?? ?? ?? ?? 8b 15 ?? ?? ?? ?? 89 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? a3 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 83 c1 04 } //1
	condition:
