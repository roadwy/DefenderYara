...
rule Trojan_Win32_CryptInject_PI_MTB_2{
	...
	strings :
		$a_02_0 = {55 8b ec 57 a1 ?? ?? ?? 00 a3 ?? ?? ?? 00 8b 0d ?? ?? ?? 00 89 0d ?? ?? ?? 00 8b 15 ?? ?? ?? 00 8b 02 a3 ?? ?? ?? 00 8b 0d ?? ?? ?? 00 81 e9 fc 1a 01 00 89 0d ?? ?? ?? 00 8b 0d ?? ?? ?? 00 81 c1 fc 1a 01 00 a1 ?? ?? ?? 00 a3 ?? ?? ?? 00 90 08 00 03 a1 ?? ?? ?? 00 31 0d ?? ?? ?? 00 [0-a0] 8b ff c7 05 ?? ?? ?? 00 00 00 00 00 a1 ?? ?? ?? 00 01 05 ?? ?? ?? 00 8b ff 8b 15 ?? ?? ?? 00 a1 ?? ?? ?? 00 89 02 5f 5d c3 } //10
		$a_02_1 = {55 8b ec 53 57 a1 ?? ?? ?? ?? a3 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 89 0d ?? ?? ?? ?? 8b 15 ?? ?? ?? ?? 8b 02 a3 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 81 e9 ?? ?? ?? ?? 89 0d ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 81 c1 ?? ?? ?? ?? a1 ?? ?? ?? ?? a3 ?? ?? ?? ?? 90 08 00 03 a1 ?? ?? ?? ?? 33 c1 90 08 00 03 8b ff c7 05 ?? ?? ?? ?? 00 00 00 00 01 05 ?? ?? ?? ?? 8b ff 8b 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 89 02 } //10
