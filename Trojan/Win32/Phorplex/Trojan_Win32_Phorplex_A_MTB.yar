...
rule Trojan_Win32_Phorplex_A_MTB{
	...
	strings :
		$a_03_0 = {6a 6b 58 6a 65 5a 6a 72 a2 ?? ?? ?? ?? 58 6a 6e a2 ?? ?? ?? ?? 58 6a 6c a2 ?? ?? ?? ?? 58 6a 33 59 6a 32 88 0d ?? ?? ?? ?? 59 6a 2e 88 0d ?? ?? ?? ?? 59 6a 64 88 0d ?? ?? ?? ?? 59 6a 6b 5f 6a 72 66 89 3d ?? ?? ?? ?? 5f 6a 6e 66 89 3d ?? ?? ?? ?? 5f 6a 33 88 15 ?? ?? ?? ?? 88 15 ?? ?? ?? ?? 66 89 15 ?? ?? ?? ?? 66 89 15 } //1
