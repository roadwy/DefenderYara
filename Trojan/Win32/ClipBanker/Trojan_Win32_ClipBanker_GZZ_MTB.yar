...
rule Trojan_Win32_ClipBanker_GZZ_MTB_2{
	...
	strings :
		$a_03_0 = {6a 00 ff 15 ?? ?? ?? ?? 85 c0 74 ?? 6a 0d ff 15 ?? ?? ?? ?? 89 45 e4 8b 45 e4 50 ff 15 ?? ?? ?? ?? 89 45 e0 8b 4d e0 51 } //10
...
rule Trojan_Win32_ClipBanker_GZZ_MTB_3{
	...
	strings :
		$a_03_0 = {8b f8 8d 44 24 ?? 50 57 ff 15 ?? ?? ?? ?? ff 15 ?? ?? ?? ?? 57 6a 01 ff 15 ?? ?? ?? ?? 56 ff d3 8d 4c 24 ?? 51 8d 4c 24 ?? e8 ?? ?? ?? ?? ff 15 } //10
		$a_03_1 = {8b f8 8b 54 24 ?? 52 57 ff 15 ?? ?? ?? ?? ff 15 ?? ?? ?? ?? 57 6a 01 ff 15 ?? ?? ?? ?? 56 ff 15 ?? ?? ?? ?? 6a 00 ff 15 ?? ?? ?? ?? 33 f6 ff 15 } //10
