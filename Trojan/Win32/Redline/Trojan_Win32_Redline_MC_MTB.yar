...
rule Trojan_Win32_Redline_MC_MTB{
	...
	strings :
		$a_03_0 = {f6 2f 47 e2 90 0a 37 00 f6 17 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 80 07 ?? 80 2f ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? f6 2f 47 e2 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win32_Redline_MC_MTB_3{
	...
	strings :
		...
		$a_03_1 = {0f b6 4d d7 8b 45 d8 33 d2 be 04 00 00 00 f7 f6 0f b6 92 ?? ?? ?? ?? 33 ca 88 4d df 8b 45 d8 8a 88 ?? ?? ?? ?? 88 4d d6 0f b6 55 df 8b 45 d8 0f b6 88 ?? ?? ?? ?? 03 ca 8b 55 d8 88 8a ?? ?? ?? ?? 68 } //5
	condition:
...
rule Trojan_Win32_Redline_MC_MTB_4{
	...
	strings :
		$a_03_0 = {8b c8 c1 ea 05 03 54 24 1c c1 e1 04 03 4c 24 20 03 c3 33 d1 33 d0 2b f2 8b ce c1 e1 04 c7 05 ?? ?? ?? ?? ?? ?? ?? ?? 89 4c 24 10 8b 44 24 24 01 44 24 10 81 3d ?? ?? ?? ?? be 01 00 00 8d 3c 33 75 } //5
		$a_03_1 = {8b c6 c1 e8 05 03 c5 33 c7 31 44 24 10 c7 05 ?? ?? ?? ?? ?? ?? ?? ?? c7 05 ?? ?? ?? ?? ?? ?? ?? ?? 8b 44 24 10 29 44 24 14 } //5
	condition:
