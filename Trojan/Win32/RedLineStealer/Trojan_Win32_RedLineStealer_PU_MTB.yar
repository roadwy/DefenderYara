...
rule Trojan_Win32_RedLineStealer_PU_MTB{
	...
	strings :
		$a_03_0 = {d3 e8 89 44 24 ?? 8b 44 24 ?? 01 44 24 ?? 8b 44 24 ?? 33 44 24 ?? 89 74 24 ?? 89 44 24 ?? 89 44 24 ?? 8b 44 24 ?? 89 44 24 ?? 8b 44 24 ?? 31 44 24 ?? 8b 4c 24 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win32_RedLineStealer_PU_MTB_2{
	...
	strings :
		$a_03_0 = {fe 40 47 4e c7 84 24 ?? ?? ?? ?? 1a 41 9f 17 c7 84 24 ?? ?? ?? ?? 44 55 93 01 c7 84 24 ?? ?? ?? ?? 79 16 54 13 c7 84 24 ?? ?? ?? ?? 7f 0c 54 3c c7 84 24 ?? ?? ?? ?? f8 dc bd 0f c7 84 24 ?? ?? ?? ?? 37 1e d5 38 } //1
