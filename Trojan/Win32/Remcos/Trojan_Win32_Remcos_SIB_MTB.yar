...
rule Trojan_Win32_Remcos_SIB_MTB{
	...
	strings :
		...
		$a_03_1 = {ba 01 00 00 00 a1 ?? ?? ?? ?? 8b 38 ff 57 ?? 8b 45 ?? 8b 16 0f b6 7c 10 ?? a1 ?? ?? ?? ?? e8 ?? ?? ?? ?? ba 00 01 00 00 2b d0 52 a1 ?? ?? ?? ?? e8 ?? ?? ?? ?? 5a 92 8b ca 99 f7 f9 03 fa 8b d7 8d 45 ?? e8 ?? ?? ?? ?? 8b 55 90 1b 08 b8 ?? ?? ?? ?? e8 ?? ?? ?? ?? ff 06 ff 4d ?? 75 } //1
	condition:
