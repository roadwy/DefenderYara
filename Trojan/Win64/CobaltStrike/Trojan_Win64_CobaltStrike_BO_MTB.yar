...
rule Trojan_Win64_CobaltStrike_BO_MTB_2{
	...
	strings :
		$a_03_0 = {48 8b 1f e8 ?? ?? ?? ?? 33 d2 48 98 48 2b f5 48 f7 f6 49 8b 07 fe c2 41 32 14 06 42 88 14 33 48 8b 0f 46 30 24 31 49 ff c6 49 8b 77 ?? 49 8b 2f 48 8b ce 48 2b cd 4c 3b f1 72 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win64_CobaltStrike_BO_MTB_3{
	...
	strings :
		...
		$a_03_3 = {48 01 c8 88 10 8b 85 ?? ?? ?? ?? 89 c2 8b 85 ?? ?? ?? ?? 88 54 05 ?? 83 85 ?? ?? ?? ?? 01 eb 90 0a 40 00 8b 95 ?? ?? ?? ?? 8b 8d ?? ?? ?? ?? 48 8b 85 } //1
	condition:
