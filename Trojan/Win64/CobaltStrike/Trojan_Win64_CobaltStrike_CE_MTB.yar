...
rule Trojan_Win64_CobaltStrike_CE_MTB_2{
	meta:
		description = "Trojan:Win64/CobaltStrike.CE!MTB,SIGNATURE_TYPE_PEHSTR_EXT,02 00 02 00 01 00 00 02 00 "
		
	strings :
		$a_03_0 = {48 98 4c 89 ca 48 29 c2 48 8b 45 90 01 01 48 01 d0 0f b6 10 8b 45 90 01 01 01 d0 44 31 c0 88 01 83 45 90 00 } //00 00 
	condition:
		any of ($a_*)
...
rule Trojan_Win64_CobaltStrike_CE_MTB_4{
	meta:
		description = "Trojan:Win64/CobaltStrike.CE!MTB,SIGNATURE_TYPE_PEHSTR_EXT,04 00 04 00 02 00 00 02 00 "
		
	strings :
		$a_03_0 = {31 c6 89 f1 48 8b 15 90 01 03 00 8b 45 fc 48 98 48 01 d0 89 ca 88 10 83 45 fc 01 eb 90 00 } //02 00 
		$a_03_1 = {89 45 fc 8b 45 fc 41 b9 04 00 00 00 41 b8 00 30 00 00 48 89 c2 b9 00 00 00 00 48 8b 05 90 01 03 00 ff d0 48 89 45 90 00 } //00 00 
	condition:
		any of ($a_*)
