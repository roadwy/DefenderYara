...
rule Trojan_Win64_IcedID_EH_MTB{
	...
	strings :
		...
		$a_03_1 = {f0 00 22 20 0b 02 ?? ?? 00 78 05 00 00 04 } //1
	condition:
...
rule Trojan_Win64_IcedID_EH_MTB_3{
	...
	strings :
		$a_03_0 = {48 8b 4c 24 40 48 03 c8 [0-03] 74 } //3
...
rule Trojan_Win64_IcedID_EH_MTB_12{
	...
	strings :
		$a_03_0 = {41 31 04 0c 49 83 c4 ?? 8b 56 ?? 8b 8e ?? ?? ?? ?? 8b 86 ?? ?? ?? ?? 03 ca 2b 86 ?? ?? ?? ?? 83 f1 ?? 01 46 ?? 2b d1 8b 06 29 86 ?? ?? ?? ?? 89 56 ?? 8b 06 8b 4e ?? 33 8e ?? ?? ?? ?? 81 c1 ?? ?? ?? ?? 0f af c1 89 06 8b 86 ?? ?? ?? ?? 01 46 ?? b8 ?? ?? ?? ?? 2b 86 ?? ?? ?? ?? 01 86 ?? ?? ?? ?? 8b 46 ?? 2b 86 ?? ?? ?? ?? 8b 4e ?? 83 c0 ?? 31 86 ?? ?? ?? ?? 2b 8e ?? ?? ?? ?? 8b 86 ?? ?? ?? ?? 81 c1 ?? ?? ?? ?? 0f af c1 89 86 ?? ?? ?? ?? 49 81 fc ?? ?? ?? ?? 0f 8c } //1
	condition:
		((#a_03_0  & 1)*1) >=1
