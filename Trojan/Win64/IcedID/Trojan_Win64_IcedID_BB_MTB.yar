...
rule Trojan_Win64_IcedID_BB_MTB{
	...
	strings :
		$a_03_0 = {8b c1 33 86 ?? ?? ?? ?? 41 2b cb 01 86 ?? ?? ?? ?? 03 cf 01 8e ?? ?? ?? ?? 8b 8e ?? ?? ?? ?? 8b 86 ?? ?? ?? ?? 44 8b 96 ?? ?? ?? ?? 2b c2 05 ?? ?? ?? ?? 01 06 b8 ?? ?? ?? ?? 2b c1 2b c3 01 46 ?? 8d 41 ?? 31 86 ?? ?? ?? ?? 49 81 f9 ?? ?? ?? ?? 0f 8c } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_Win64_IcedID_BB_MTB_3{
	...
	strings :
		$a_03_0 = {44 01 c0 44 01 e0 44 29 f8 48 98 32 14 06 48 8b 44 24 ?? 88 14 38 48 39 5c 24 ?? 48 8d 43 01 48 89 44 24 ?? 90 13 8b 0d ?? ?? ?? ?? 44 8b 15 ?? ?? ?? ?? 44 8b 05 ?? ?? ?? ?? 44 8b 0d ?? ?? ?? ?? 44 8b 1d ?? ?? ?? ?? 8b 35 ?? ?? ?? ?? 89 c8 44 89 d7 41 0f af c2 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
