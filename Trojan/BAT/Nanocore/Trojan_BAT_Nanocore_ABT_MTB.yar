...
rule Trojan_BAT_Nanocore_ABT_MTB{
	...
	strings :
		$a_03_0 = {91 61 d2 6f ?? ?? ?? 0a 07 1d 2c 04 17 58 0b 07 02 8e 69 32 db 06 6f ?? ?? ?? 0a 25 2d 02 26 14 2a } //2
...
rule Trojan_BAT_Nanocore_ABT_MTB_2{
	...
	strings :
		$a_03_0 = {16 0d 2b 31 00 07 08 09 28 ?? ?? ?? 06 28 ?? ?? ?? 06 00 28 ?? ?? ?? 06 28 ?? ?? ?? 06 28 ?? ?? ?? 06 00 7e ?? ?? ?? 04 06 28 ?? ?? ?? 06 d2 9c 00 09 17 58 0d 09 17 fe 04 13 04 11 04 2d c5 06 17 58 0a 00 08 17 58 0c 08 20 ?? ?? ?? 00 fe 04 13 05 11 05 2d a9 28 ?? ?? ?? 0a 7e ?? ?? ?? 04 6f ?? ?? ?? 0a 28 ?? ?? ?? 06 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 80 ?? ?? ?? 04 2a } //4
