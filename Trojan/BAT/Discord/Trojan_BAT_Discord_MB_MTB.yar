...
rule Trojan_BAT_Discord_MB_MTB{
	...
	strings :
		$a_02_0 = {fa 01 33 00 16 ?? ?? 01 ?? ?? ?? 36 ?? ?? ?? 05 ?? ?? ?? 0a ?? ?? ?? 0f ?? ?? ?? 07 ?? ?? ?? 4e ?? ?? ?? 13 } //10
...
rule Trojan_BAT_Discord_MB_MTB_2{
	...
	strings :
		$a_03_0 = {0a 06 1f 20 28 ?? ?? ?? 2b 28 ?? ?? ?? 2b 0b 06 1f 20 28 ?? ?? ?? 2b 1f 20 28 ?? ?? ?? 2b 28 ?? ?? ?? 2b 0c 06 1f 40 28 ?? ?? ?? 2b 06 8e 69 1f 40 59 28 ?? ?? ?? 2b 28 ?? ?? ?? 2b 0d 03 07 20 e8 03 00 00 73 ?? ?? ?? 0a 13 04 } //1
