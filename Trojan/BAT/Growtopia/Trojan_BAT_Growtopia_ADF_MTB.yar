...
rule Trojan_BAT_Growtopia_ADF_MTB{
	...
	strings :
		$a_02_0 = {0a 0d 06 72 ?? ?? ?? 70 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 13 04 09 11 04 11 04 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 58 6f ?? ?? ?? 0a 17 8d ?? ?? ?? 01 13 06 11 06 16 1f 20 9d 11 06 6f ?? ?? ?? 0a 19 9a 7e ?? ?? ?? 0a 6f ?? ?? ?? 0a 25 2d 06 26 72 ?? ?? ?? 70 13 05 de 0a } //10
