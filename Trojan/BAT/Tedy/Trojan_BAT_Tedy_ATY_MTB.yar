...
rule Trojan_BAT_Tedy_ATY_MTB{
	...
	strings :
		$a_03_0 = {01 25 16 7e ?? 00 00 04 a2 25 17 7e ?? 00 00 04 a2 0b 06 14 28 ?? 00 00 0a 2c 12 06 14 17 8d ?? ?? ?? 01 25 16 07 a2 } //2
...
rule Trojan_BAT_Tedy_ATY_MTB_2{
	...
	strings :
		$a_03_0 = {16 0a 2b 0d 02 06 02 06 91 03 61 d2 9c 06 17 58 0a 06 02 28 ?? 00 00 06 25 26 69 32 e7 } //1
...
rule Trojan_BAT_Tedy_ATY_MTB_3{
	...
	strings :
		$a_03_0 = {72 01 00 00 70 6f ?? ?? ?? 0a 06 72 1f 00 00 70 6f ?? ?? ?? 0a 06 17 6f ?? ?? ?? 0a 06 17 6f ?? ?? ?? 0a 06 16 6f ?? ?? ?? 0a 06 17 6f ?? ?? ?? 0a 73 16 00 00 0a 25 06 } //2
