...
rule Trojan_BAT_CobaltStrike_ZG_MTB{
	...
	strings :
		$a_03_0 = {00 28 13 00 00 0a 03 6f ?? ?? ?? 0a 0a 7e ?? ?? ?? 04 0b 02 28 ?? ?? ?? 0a 0c 73 ?? ?? ?? 0a 0d 73 ?? ?? ?? 0a 13 04 11 04 09 06 07 6f ?? ?? ?? 0a 17 73 ?? ?? ?? 0a 13 05 11 05 08 16 08 8e 69 6f ?? ?? ?? 0a 00 11 05 6f ?? ?? ?? 0a 00 28 ?? ?? ?? 0a 11 04 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 13 06 dd ?? ?? ?? 00 } //1
