...
rule Trojan_BAT_Scarsi_ABIP_MTB{
	...
	strings :
		$a_03_0 = {09 11 05 09 6f ?? ?? ?? 0a 1e 5b 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 09 17 6f ?? ?? ?? 0a 06 13 06 08 09 6f ?? ?? ?? 0a 17 73 ?? ?? ?? 0a 13 07 11 07 11 06 16 11 06 8e 69 6f ?? ?? ?? 0a de 0c 11 07 2c 07 11 07 6f ?? ?? ?? 0a dc 08 6f ?? ?? ?? 0a 13 08 de 14 } //3
