...
rule Trojan_BAT_Njrat_MG_MTB{
	...
	strings :
		$a_03_0 = {11 04 03 09 6f ?? ?? ?? 0a 13 06 12 06 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a 03 09 6f ?? ?? ?? 0a 28 ?? ?? ?? 0a 16 28 ?? ?? ?? 0a 16 fe 01 fe 01 13 05 11 05 2c 1e 07 03 09 6f ?? ?? ?? 0a 13 06 12 06 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 26 2b 0e 07 03 09 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 26 00 09 17 d6 0d 09 08 3e 3e } //10
