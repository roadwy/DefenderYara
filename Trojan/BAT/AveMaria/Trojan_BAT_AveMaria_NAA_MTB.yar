...
rule Trojan_BAT_AveMaria_NAA_MTB{
	...
	strings :
		$a_03_0 = {07 20 1f d8 36 76 5a 20 ?? ?? ?? 98 61 38 ?? ?? ?? ff 00 07 20 ?? ?? ?? 5f 5a 20 ?? ?? ?? 69 61 38 ?? ?? ?? ff 02 7b ?? ?? ?? 04 1a 28 ?? ?? ?? 06 } //5
...
rule Trojan_BAT_AveMaria_NAA_MTB_2{
	...
	strings :
		$a_03_0 = {28 9c 00 00 06 0d 7e ?? ?? ?? 04 09 02 16 02 8e 69 28 ?? ?? ?? 06 2a 73 ?? ?? ?? 0a 38 ?? ?? ?? ff 0a 38 ?? ?? ?? ff 0b 38 ?? ?? ?? ff 73 ?? ?? ?? 0a 38 ?? ?? ?? ff 28 ?? ?? ?? 06 } //5
		$a_03_1 = {06 1f 20 02 7e ?? ?? 00 04 20 ?? ?? 00 00 28 ?? ?? 00 06 28 ?? ?? 00 06 0a 02 7b ?? ?? 00 04 14 06 28 ?? ?? 00 06 26 20 ?? ?? 00 00 } //5
...
rule Trojan_BAT_AveMaria_NAA_MTB_3{
	...
	strings :
		$a_03_0 = {28 8b 01 00 06 61 7e ?? ?? 00 04 28 ?? ?? 00 06 11 01 11 03 17 58 11 01 8e 69 5d 91 7e ?? ?? 00 04 28 ?? ?? 00 06 59 20 ?? ?? 00 00 58 20 ?? ?? 00 00 5d 7e ?? ?? 00 04 28 ?? ?? 00 06 9c } //5
