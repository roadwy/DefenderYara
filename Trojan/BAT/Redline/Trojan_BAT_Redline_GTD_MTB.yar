...
rule Trojan_BAT_Redline_GTD_MTB{
	...
	strings :
		$a_03_0 = {02 1a 58 11 04 16 08 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 11 04 16 11 04 8e 69 6f ?? ?? ?? 0a 13 05 7e ?? ?? ?? ?? 11 05 6f ?? ?? ?? 0a 7e ?? ?? ?? ?? 02 6f ?? ?? ?? 0a 7e ?? ?? ?? ?? 6f ?? ?? ?? 0a 17 59 28 ?? ?? ?? 0a 16 7e ?? ?? ?? ?? 02 1a 28 ?? ?? ?? 0a 11 05 0d } //10
