...
rule Trojan_BAT_RedLine_MH_MTB{
	...
	strings :
		$a_03_0 = {11 09 11 03 16 11 03 8e 69 6f ?? ?? ?? 0a 13 06 38 ?? ?? ?? ?? 14 13 06 20 01 00 00 00 28 ?? ?? ?? 06 3a ?? ?? ?? ?? 26 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_BAT_RedLine_MH_MTB_2{
	...
	strings :
		$a_03_0 = {08 11 07 07 11 07 9a 1f 10 28 ?? ?? ?? 0a 9c 11 07 17 58 13 07 11 07 07 8e 69 fe 04 13 08 11 08 } //5
