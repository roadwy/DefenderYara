...
rule Trojan_BAT_Redline_UW_MTB{
	...
	strings :
		$a_03_0 = {13 05 19 8d ?? ?? ?? 01 13 13 11 13 16 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a a2 11 13 17 7e ?? ?? ?? 0a a2 11 13 18 09 11 05 6f ?? ?? ?? 0a a2 11 13 13 06 72 ?? ?? ?? 70 28 ?? ?? ?? 06 28 ?? ?? ?? 06 13 07 28 ?? ?? ?? 0a 11 07 6f ?? ?? ?? 0a 13 08 72 ?? ?? ?? 70 13 09 11 09 } //10
