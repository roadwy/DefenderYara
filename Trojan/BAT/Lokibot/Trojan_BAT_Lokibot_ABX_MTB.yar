
rule Trojan_BAT_Lokibot_ABX_MTB{
	meta:
		description = "Trojan:BAT/Lokibot.ABX!MTB,SIGNATURE_TYPE_PEHSTR_EXT,08 00 08 00 04 00 00 05 00 "
		
	strings :
		$a_03_0 = {16 0d 2b 31 00 07 08 09 28 90 01 03 06 28 90 01 03 06 00 28 90 01 03 06 28 90 01 03 06 28 90 01 03 06 00 7e 90 01 03 04 06 28 90 01 03 06 d2 9c 00 09 17 58 0d 09 17 fe 04 13 04 11 04 2d c5 06 17 58 0a 00 08 17 58 0c 08 20 90 01 03 00 fe 04 13 05 11 05 2d a9 28 90 01 03 0a 7e 90 01 03 04 6f 90 01 03 0a 28 90 01 03 06 72 90 01 03 70 6f 90 01 03 0a 80 90 01 03 04 2a 90 00 } //01 00 
		$a_01_1 = {43 72 65 61 74 65 49 6e 73 74 61 6e 63 65 } //01 00  CreateInstance
		$a_01_2 = {57 00 68 00 61 00 6c 00 65 00 } //01 00  Whale
		$a_01_3 = {47 00 61 00 6d 00 65 00 4e 00 65 00 74 00 77 00 6f 00 72 00 6b 00 2e 00 50 00 72 00 6f 00 70 00 65 00 72 00 74 00 69 00 65 00 73 00 2e 00 52 00 65 00 73 00 6f 00 75 00 72 00 63 00 65 00 73 00 } //00 00  GameNetwork.Properties.Resources
	condition:
		any of ($a_*)
 
}