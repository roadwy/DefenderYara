...
rule Trojan_BAT_Remcos_PP_MTB{
	...
	strings :
		$a_03_0 = {07 11 06 11 07 6f ?? ?? ?? 0a 13 08 08 12 08 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a 08 6f ?? ?? ?? 0a 20 ?? ?? ?? 00 2f 0d 08 12 08 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a 08 6f ?? ?? ?? 0a 20 ?? ?? ?? 00 2f 0d 08 12 08 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a 11 07 17 58 13 07 11 07 07 6f ?? ?? ?? 0a 32 a3 } //2
	condition:
		((#a_03_0  & 1)*2) >=2
