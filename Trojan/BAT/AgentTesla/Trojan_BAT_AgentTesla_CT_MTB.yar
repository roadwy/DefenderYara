...
rule Trojan_BAT_AgentTesla_CT_MTB{
	...
	strings :
		$a_02_0 = {06 20 00 01 00 00 6f ?? ?? ?? 0a 06 20 ?? ?? ?? 00 6f ?? ?? ?? 0a 7e 04 00 00 04 7e 03 00 00 04 20 ?? ?? ?? 00 73 [0-06] 2d 43 26 06 07 06 6f ?? ?? ?? 0a 1e 5b 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 06 07 06 6f ?? ?? ?? 0a 1e 5b 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 06 17 6f ?? ?? ?? 0a 02 06 6f } //10
