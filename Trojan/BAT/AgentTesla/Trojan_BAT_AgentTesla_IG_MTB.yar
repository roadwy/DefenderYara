...
rule Trojan_BAT_AgentTesla_IG_MTB{
	...
	strings :
		$a_02_0 = {38 88 02 00 00 [0-40] 38 ?? ?? ?? 00 02 7b ?? ?? ?? 04 02 7b ?? ?? ?? 04 02 7b ?? ?? ?? 04 9e 38 ?? ?? ?? 00 02 7b ?? ?? ?? 04 02 7b ?? ?? ?? 04 02 7b ?? ?? ?? 04 02 7b ?? ?? ?? 04 94 9e 38 } //10
...
rule Trojan_BAT_AgentTesla_IG_MTB_2{
	...
	strings :
		$a_03_0 = {0a 06 16 20 00 b0 02 00 8d ?? ?? ?? ?? a2 06 17 00 20 67 02 00 00 20 4a 02 00 00 28 ?? ?? ?? ?? a2 06 18 06 17 9a 14 } //1
