...
rule Trojan_BAT_AgentTesla_LVM_MTB{
	...
	strings :
		$a_03_0 = {0a 0b 06 16 73 ?? ?? ?? 0a 73 ?? ?? ?? 0a 0c 08 07 6f ?? ?? ?? 0a 07 6f ?? ?? ?? 0a 80 ?? ?? ?? 04 de 1e } //1
		$a_03_1 = {16 0d 2b 30 08 09 9a 13 04 11 04 6f ?? ?? ?? 0a 72 ?? ?? ?? 70 28 ?? ?? ?? 0a 2c 14 11 04 14 14 6f ?? ?? ?? 0a 26 72 ?? ?? ?? 70 28 ?? ?? ?? 0a 09 17 58 0d 09 08 8e 69 32 ca 07 17 58 0b 07 06 8e 69 32 b3 } //1
