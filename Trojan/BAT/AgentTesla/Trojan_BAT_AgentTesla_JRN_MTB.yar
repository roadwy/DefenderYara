...
rule Trojan_BAT_AgentTesla_JRN_MTB{
	...
	strings :
		$a_03_0 = {09 11 04 9a 13 05 11 05 28 ?? ?? ?? 0a 23 ?? ?? ?? ?? ?? ?? ?? ?? 59 28 ?? ?? ?? 0a b7 13 06 07 11 06 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 0b 00 11 04 17 d6 13 04 11 04 09 8e 69 fe 04 13 07 11 07 2d ba } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule Trojan_BAT_AgentTesla_JRN_MTB_2{
	...
	strings :
		$a_03_0 = {0a 0b 09 28 ?? ?? ?? 0a 03 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 13 04 06 11 04 6f ?? ?? ?? 0a 06 18 6f ?? ?? ?? 0a 02 28 ?? ?? ?? 0a 0c 28 ?? ?? ?? 0a 06 6f ?? ?? ?? 0a 08 16 08 8e 69 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 0b 07 13 05 11 05 } //1
