...
rule Trojan_BAT_AgentTesla_PSD_MTB{
	...
	strings :
		$a_03_0 = {72 c7 01 00 70 11 04 8c 0c ?? ?? ?? 28 2a ?? ?? ?? 28 1b ?? ?? ?? 73 2b ?? ?? ?? 25 72 f3 01 00 70 6f 2c ?? ?? ?? 25 72 03 02 00 70 28 01 ?? ?? ?? 28 2a ?? ?? ?? 6f 2d ?? ?? ?? 25 17 6f 2e ?? ?? ?? 25 17 6f 2f ?? ?? ?? 25 16 6f 30 ?? ?? ?? 25 17 6f 31 ?? ?? ?? 28 32 ?? ?? ?? 26 28 01 ?? ?? ?? 14 1a 28 0a ?? ?? ?? 26 16 2a } //5
...
rule Trojan_BAT_AgentTesla_PSD_MTB_2{
	...
	strings :
		$a_03_0 = {7e 9c 00 00 04 d0 0e ?? ?? ?? 28 0e ?? ?? ?? 6f a3 ?? ?? ?? 2c 25 20 2e 17 c4 03 28 df ?? ?? ?? 16 8d a7 ?? ?? ?? 28 a4 ?? ?? ?? 73 a5 ?? ?? ?? 7a 73 a6 ?? ?? ?? 80 9c ?? ?? ?? 7e 9c ?? ?? ?? d0 0e ?? ?? ?? 28 0e ?? ?? ?? 14 6f a7 ?? ?? ?? 28 ?? ?? ?? 2b 0a } //5
