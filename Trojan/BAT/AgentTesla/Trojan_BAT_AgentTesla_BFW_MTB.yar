...
rule Trojan_BAT_AgentTesla_BFW_MTB{
	...
	strings :
		$a_02_0 = {0a 0b 00 00 07 0a 38 [0-05] 72 ?? ?? ?? 70 03 11 05 18 6f ?? ?? ?? 0a 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 04 09 6f ?? ?? ?? 0a 28 ?? ?? ?? 0a 6a 61 b7 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 13 0a 17 13 13 11 0a 13 06 00 2b 00 00 08 11 06 6f ?? ?? ?? 0a 26 09 04 6f } //10
