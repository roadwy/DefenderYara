...
rule Trojan_BAT_AgentTesla_CJ_MTB_2{
	...
	strings :
		$a_03_0 = {02 1a 58 11 04 16 08 28 4a ?? ?? ?? 28 47 ?? ?? ?? 11 04 16 11 04 8e 69 6f 8a ?? ?? ?? 13 05 7e 19 ?? ?? ?? 11 05 6f 8b ?? ?? ?? 7e 1f ?? ?? ?? 02 6f 8c ?? ?? ?? 7e 19 ?? ?? ?? 6f 8d ?? ?? ?? 17 59 28 8e ?? ?? ?? 16 7e 1d ?? ?? ?? 02 1a 28 4a ?? ?? ?? 11 05 } //10
