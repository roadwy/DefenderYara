...
rule Trojan_BAT_AgentTesla_ABR_MTB_2{
	...
	strings :
		$a_03_0 = {06 13 08 11 07 11 08 16 11 08 8e 69 6f ?? ?? ?? 0a 28 ?? ?? ?? 0a 09 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 13 05 de 16 90 0a 2e 00 72 ?? ?? ?? 70 28 03 } //6
...
rule Trojan_BAT_AgentTesla_ABR_MTB_3{
	...
	strings :
		$a_03_0 = {07 1f 10 8d ?? ?? ?? 01 25 d0 ?? ?? ?? 04 28 ?? ?? ?? 06 6f ?? ?? ?? 0a 06 07 6f ?? ?? ?? 0a 17 73 ?? ?? ?? 0a 0c 08 02 16 02 8e 69 6f ?? ?? ?? 0a 08 6f ?? ?? ?? 0a 06 28 ?? ?? ?? 06 0d 28 ?? ?? ?? 06 09 2a } //1
