...
rule Trojan_BAT_AgentTesla_BHT_MTB{
	...
	strings :
		$a_02_0 = {0a 0b 07 74 ?? ?? ?? 01 16 73 ?? ?? ?? 0a 0c 1a 8d ?? ?? ?? 01 0d 07 14 72 ?? ?? ?? 70 17 8d ?? ?? ?? 01 25 16 07 14 72 ?? ?? ?? 70 16 8d ?? ?? ?? 01 14 14 14 28 ?? ?? ?? 0a 1b 8c ?? ?? ?? 01 28 ?? ?? ?? 0a a2 14 14 28 ?? ?? ?? 0a 00 07 14 72 ?? ?? ?? 70 19 8d } //10
