...
rule Trojan_BAT_AgentTesla_BXB_MTB{
	...
	strings :
		$a_02_0 = {0a 1f 64 73 ?? ?? ?? 0a 1f 10 6f ?? ?? ?? 0a 28 ?? ?? ?? 0a 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 17 73 ?? ?? ?? 0a 0b 07 02 16 02 8e 69 6f ?? ?? ?? 0a 07 6f ?? ?? ?? 0a de 0a 07 2c 06 07 6f ?? ?? ?? 0a dc 06 6f ?? ?? ?? 0a 0c de 0a } //10
		$a_02_1 = {07 1f 64 73 ?? ?? ?? 0a 0c 73 ?? ?? ?? 0a 0d 09 20 00 01 00 00 6f ?? ?? ?? 0a 09 17 6f ?? ?? ?? 0a 03 2d 11 09 08 1f 10 6f ?? ?? ?? 0a 06 6f ?? ?? ?? 0a 2b 0f 09 08 1f 10 6f ?? ?? ?? 0a 06 6f ?? ?? ?? 0a 13 04 73 ?? ?? ?? 0a 13 05 } //10
