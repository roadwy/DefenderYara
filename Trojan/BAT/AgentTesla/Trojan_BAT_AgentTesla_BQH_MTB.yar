...
rule Trojan_BAT_AgentTesla_BQH_MTB{
	...
	strings :
		$a_02_0 = {70 18 1b 8d ?? ?? ?? 01 25 16 72 ?? ?? ?? 70 a2 25 17 20 00 01 00 00 8c ?? ?? ?? 01 a2 25 1a 17 8d ?? ?? ?? 01 25 16 03 74 ?? ?? ?? 1b 28 ?? ?? ?? 06 a2 a2 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 0a 02 06 28 ?? ?? ?? 0a 72 ?? ?? ?? 70 18 17 8d ?? ?? ?? 01 25 16 72 ?? ?? ?? 70 a2 28 } //10
