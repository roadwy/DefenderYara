...
rule Trojan_BAT_FormBook_FG_MTB{
	...
	strings :
		$a_02_0 = {0a 13 04 72 ?? ?? ?? 70 72 ?? ?? ?? 70 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 72 ?? ?? ?? 70 72 ?? ?? ?? 70 6f ?? ?? ?? 0a 13 05 08 09 18 17 8d 01 00 00 01 13 08 11 08 16 07 a2 11 08 28 ?? ?? ?? 0a 13 06 11 06 11 04 18 16 8d 01 00 00 01 28 ?? ?? ?? 0a 13 07 11 07 11 05 17 18 8d 01 00 00 01 13 09 11 09 16 16 8c 15 00 00 01 a2 11 09 28 ?? ?? ?? 0a 26 2a } //10
