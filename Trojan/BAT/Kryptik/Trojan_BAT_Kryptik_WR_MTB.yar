...
rule Trojan_BAT_Kryptik_WR_MTB{
	...
	strings :
		$a_03_0 = {6e 17 6a d6 20 [0-04] 6a 5f b8 0c 09 11 05 08 84 95 d7 6e 20 [0-04] 6a 5f b8 0d 11 05 08 84 95 13 04 11 05 08 84 11 05 09 84 95 9e 11 05 09 84 11 04 } //10
		$a_03_1 = {9e 11 06 11 07 02 11 07 91 11 05 11 05 08 84 95 11 05 09 84 95 d7 6e 20 [0-04] 6a 5f b7 95 61 86 9c 11 07 17 d6 13 07 11 07 11 0a 31 9b } //10
	condition:
...
rule Trojan_BAT_Kryptik_WR_MTB_2{
	...
	strings :
		$a_03_0 = {25 16 09 28 [0-04] 28 ?? ?? ?? ?? a2 28 ?? ?? ?? ?? ?? ?? ?? ?? ?? 13 04 11 04 28 ?? ?? ?? ?? ?? ?? ?? ?? ?? 07 6f ?? ?? ?? ?? 18 14 28 ?? ?? ?? ?? ?? ?? ?? ?? ?? 13 05 11 05 28 ?? ?? ?? ?? ?? ?? ?? ?? ?? 08 6f ?? ?? ?? ?? 17 18 8d ?? ?? ?? ?? 25 16 72 } //10
