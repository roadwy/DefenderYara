...
rule TrojanSpy_Win32_Delf_2{
	...
	strings :
		...
		$a_02_21 = {ff ff 8d 45 fc ba ?? ?? 40 00 e8 ?? ?? ff ff [0-16] 8d 45 fc e8 ?? ?? ff ff 8b d0 b9 ?? ?? 40 00 b8 00 00 00 80 e8 ?? ?? ff ff 68 ?? ?? 40 00 8d 45 fc e8 ?? ?? ff ff 8b d0 b9 ?? ?? 40 00 b8 00 00 00 80 e8 ?? ?? ff ff 68 ?? ?? 40 00 ?? ?? ?? 40 00 [0-05] b8 02 00 00 80 e8 ?? ?? ff ff 33 c0 5a 59 59 64 89 } //5
		$a_02_22 = {e8 7f ff ff ff 8d 55 f4 52 50 e8 ?? ?? ff ff 8b 45 f4 50 6a 00 68 18 04 00 00 e8 ?? ?? ff ff 8b d8 85 db 74 42 be ?? 00 00 00 8d 45 f0 8b d6 e8 ?? ?? ff ff 8d 45 f8 50 56 8b 45 f0 e8 ?? ?? ff ff 50 8b 45 fc 50 53 e8 ?? ?? ff ff 85 c0 74 11 8b 45 f0 } //8
		...
		$a_02_24 = {ff ff 8d 45 d8 50 68 80 00 00 00 6a 04 53 e8 ?? ?? ff ff 8d 45 e4 50 6a 04 8d 45 08 50 53 e8 ?? fa ff ff 50 e8 ?? fa ff ff 8d 45 e0 50 8b 45 d8 50 6a 04 53 e8 ?? fa ff ff } //8
		...
		$a_02_26 = {ff ff 8b f0 57 a1 50 ?? 40 00 50 e8 ?? ?? ff ff ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 50 56 e8 ?? ed ff ff 56 e8 ?? ?? ff ff ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 56 e8 ?? ?? ff ff 8b c3 e8 ?? ?? ff ff 53 e8 ?? ?? ff ff } //8
