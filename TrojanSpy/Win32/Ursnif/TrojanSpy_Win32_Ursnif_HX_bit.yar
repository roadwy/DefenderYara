...
rule TrojanSpy_Win32_Ursnif_HX_bit{
	...
	strings :
		$a_03_0 = {8b 02 85 c0 8b f0 74 ?? 33 c1 33 44 24 10 43 8a cb d3 c8 8b ce 89 02 83 c2 04 ff 4c 24 0c 75 } //1
...
rule TrojanSpy_Win32_Ursnif_HX_bit_2{
	...
	strings :
		...
		$a_03_2 = {8b 08 69 c9 ?? ?? ?? ?? 81 c1 ?? ?? ?? ?? 89 0a 69 c9 ?? ?? ?? ?? 81 c1 ?? ?? ?? ?? 56 66 8b f1 69 c9 ?? ?? ?? ?? 66 89 72 04 be ?? ?? ?? ?? 03 ce 57 89 08 66 89 4a 06 33 ff } //1
		$a_03_3 = {8b 08 69 c9 ?? ?? ?? ?? 03 ce 88 4c 3a 08 47 89 08 83 ff 08 72 ea } //1
	condition:
