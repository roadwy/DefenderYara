...
rule TrojanSpy_Win32_Banker_ABU{
	...
	strings :
		$a_03_0 = {8b 55 f8 a1 30 2f 47 00 8b 08 ff 51 38 8d 45 f0 50 8b 0d 30 2f 47 00 ba ?? ?? ?? ?? 8b 83 00 03 00 00 e8 } //1
		$a_03_1 = {73 75 62 6a 65 63 74 3d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 6d 65 73 73 61 67 65 3d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 68 74 74 70 3a 2f 2f } //1
