...
rule TrojanSpy_Win32_Pophot_F_dll{
	...
	strings :
		$a_02_0 = {50 b9 e8 03 00 00 ba 01 00 00 00 b8 ?? ?? ?? ?? e8 ?? ?? ff ff 8b 55 ?? b9 ?? ?? ?? ?? b8 02 00 00 80 e8 ?? ?? ff ff 8b 45 fc e8 ?? ?? ff ff 83 f8 0a 0f 8f ?? ?? 00 00 8d 45 ?? 50 8d 85 7c ff ff ff 50 b9 e8 03 00 00 ba 01 00 00 00 b8 ?? ?? ?? ?? e8 ?? ?? ff ff 8b 95 7c ff ff ff b9 ?? ?? ?? ?? b8 02 00 00 80 e8 ?? ?? ff ff } //10
		$a_02_1 = {b9 e8 03 00 00 ba 01 00 00 00 b8 ?? ?? ?? ?? e8 ?? ?? ff ff 8d 45 b4 50 b9 64 00 00 00 ba 01 00 00 00 b8 ?? ?? ?? ?? e8 ?? ?? ff ff 8b 55 b4 } //10
