...
rule VirTool_WinNT_Tedroo_A{
	...
	strings :
		...
		$a_02_3 = {fa 0f 20 c0 89 45 ec 25 ff ff fe ff 0f 22 c0 8b 0d ?? ?? ?? ?? 8b 11 a1 ?? ?? ?? ?? c7 04 82 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 8b 11 a1 ?? ?? ?? ?? c7 04 82 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 8b 11 a1 ?? ?? ?? ?? c7 04 82 ?? ?? ?? ?? 8b 0d ?? ?? ?? ?? 8b 11 a1 ?? ?? ?? ?? c7 04 82 ?? ?? ?? ?? 8b 45 ec 0f 22 c0 } //10
	condition:
