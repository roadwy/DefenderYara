...
rule VirTool_Win32_DelfInject_gen_CE{
	...
	strings :
		...
		$a_03_1 = {8b 40 0c 03 c3 50 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 66 83 c0 03 8b 56 1c 03 d3 0f b7 c0 c1 e0 02 03 d0 } //1
		$a_03_2 = {6a 40 68 00 30 00 00 6a 10 6a 00 53 ff 15 ?? ?? ?? ?? 8b f0 8d 45 f8 50 6a 10 8d 45 e4 50 56 53 ff } //1
