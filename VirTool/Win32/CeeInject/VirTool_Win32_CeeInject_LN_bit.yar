...
rule VirTool_Win32_CeeInject_LN_bit{
	...
	strings :
		$a_03_0 = {f7 f9 8b 15 ?? ?? ?? ?? 88 84 15 90 09 24 00 be ?? ?? ?? ?? 8d bd ?? ?? ?? ?? b9 06 00 00 00 f3 a5 a4 0f bf 05 ?? ?? ?? ?? 0f be 15 ?? ?? ?? ?? 8b ca 99 } //1
		$a_03_1 = {f7 f9 8b 15 ?? ?? ?? ?? 88 84 15 90 09 11 00 0f bf 05 ?? ?? ?? ?? 0f be 15 ?? ?? ?? ?? 8b ca 99 } //1
