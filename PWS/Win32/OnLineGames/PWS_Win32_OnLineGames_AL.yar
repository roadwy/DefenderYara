...
rule PWS_Win32_OnLineGames_AL{
	...
	strings :
		$a_03_0 = {6a 00 50 8d ?? ?? ?? 6a 04 51 56 ff d3 8b 54 24 10 6a 00 81 c2 f8 00 00 00 6a 00 52 56 ff d7 33 c0 8d ?? ?? ?? 89 ?? ?? ?? 8d ?? ?? ?? 89 ?? ?? ?? 50 51 6a 08 52 56 66 ?? ?? ?? ?? ff d3 8b ?? ?? ?? 8b ?? ?? ?? 40 00 3d 32 54 76 98 } //5
