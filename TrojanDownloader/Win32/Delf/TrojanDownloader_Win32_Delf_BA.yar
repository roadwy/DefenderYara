...
rule TrojanDownloader_Win32_Delf_BA{
	...
	strings :
		...
		$a_02_14 = {68 74 74 70 3a 2f 2f [0-20] 2f 68 6b 2f 67 65 74 63 [0-04] 2e 70 68 70 } //1
		...
		$a_02_24 = {b3 01 6a 00 6a 00 6a 00 6a 03 68 ?? ?? ?? ?? ?? ?? ?? ?? ?? 89 45 e4 8b 45 f4 ba ?? ?? ?? ?? ?? ?? ?? ?? ?? 0f 85 ?? ?? ?? ?? 6a 00 6a 00 6a 03 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 6a 50 8b 45 d4 e8 ?? ?? ?? ?? 50 8b 45 e4 50 e8 ?? ?? ?? ?? 89 45 e8 6a 00 68 00 00 00 80 6a 00 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8b 45 d0 e8 ?? ?? ?? ?? 50 8b 45 f4 e8 ?? ?? ?? ?? 50 8b 45 e8 50 e8 ?? ?? ?? ?? 8b f0 8d 45 d8 } //1
		$a_02_25 = {8b 45 cc 50 8d 45 e8 50 8d 45 c4 e8 ?? ?? ?? ?? ff 75 c4 68 ?? ?? ?? ?? ff 75 f0 8d 45 c8 ba 03 00 00 00 e8 ?? ?? ?? ?? 8b 55 c8 b9 ?? ?? ?? ?? b8 ?? ?? ?? ?? e8 ?? ?? ?? ?? a1 ?? ?? ?? ?? ff 30 68 ?? ?? ?? ?? ff 75 f0 68 ?? ?? ?? ?? 8d 45 c0 ba 04 00 00 00 e8 ?? ?? ?? ?? 8b 45 c0 e8 ?? ?? ?? ?? 50 e8 } //1
	condition:
