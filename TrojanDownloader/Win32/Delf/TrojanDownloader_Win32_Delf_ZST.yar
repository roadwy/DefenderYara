...
rule TrojanDownloader_Win32_Delf_ZST{
	...
	strings :
		...
		$a_03_1 = {00 73 2e 62 61 74 00 90 09 23 00 22 20 67 6f 74 6f 20 61 61 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 64 65 6c 20 25 30 } //1
		$a_03_2 = {3f 74 74 6c 3d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 26 76 3d ?? ?? ?? ?? ?? ?? ?? ?? ?? 26 73 3d ?? ?? ?? ?? ?? ?? ?? ?? ?? 26 6e 3d 90 09 10 00 78 2e 61 73 70 } //1
	condition:
