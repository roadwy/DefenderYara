...
rule TrojanDownloader_Win32_Delf_2{
	...
	strings :
		$a_03_0 = {ba cc 03 45 00 e8 ?? ?? ?? ff b8 20 ea 45 00 ba 00 04 45 00 } //1
		$a_03_1 = {ba 18 04 45 00 e8 ?? ?? ?? ff b8 20 ea 45 00 ba 54 04 45 00 } //1
...
rule TrojanDownloader_Win32_Delf_3{
	...
	strings :
		...
		$a_02_3 = {43 3a 5c 41 72 71 75 69 76 6f 73 20 64 65 20 70 72 6f 67 72 61 6d 61 73 5c 4d 53 4e 20 4d 65 73 73 65 6e 67 65 72 5c 44 65 76 69 63 65 20 4d 61 6e 61 67 65 72 5c 6d 73 6e 67 72 5c [0-08] 2e 65 78 65 } //1
	condition:
...
rule TrojanDownloader_Win32_Delf_4{
	...
	strings :
		$a_02_0 = {69 6e 64 65 78 2e 61 73 70 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 69 6e 64 65 78 2e 68 74 6d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 69 6e 64 65 78 2e 68 74 6d 6c ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 69 6e 64 65 78 2e 70 68 70 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 44 65 66 61 75 6c 74 2e 61 73 70 ?? ?? ?? ?? ?? ?? ?? ?? ?? 44 65 66 61 75 6c 74 2e 68 74 6d ?? ?? ?? ?? ?? ?? ?? ?? ?? 44 65 66 61 75 6c 74 2e 68 74 6d 6c ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 44 65 66 61 75 6c 74 2e 50 48 50 } //1
		...
		$a_02_2 = {6d 79 64 6f 77 6e ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 68 74 74 70 3a 2f 2f ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 26 74 67 69 64 3d ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 26 61 64 64 72 65 73 73 3d } //1
...
rule TrojanDownloader_Win32_Delf_6{
	...
	strings :
		...
		$a_02_1 = {68 74 74 70 3a 2f 2f 38 30 2e 36 39 2e 31 36 30 2e 90 05 03 0a 30 2d 39 2f 75 70 64 61 74 65 2f } //1000
