...
rule TrojanDownloader_Win32_Zlob_gen_AI{
	meta:
		description = "TrojanDownloader:Win32/Zlob.gen!AI,SIGNATURE_TYPE_PEHSTR_EXT,78 05 14 05 15 00 00 "
	...
	strings :
		...
		$a_01_15 = {53 4f 46 54 57 41 52 45 5c 4d 69 63 72 6f 73 6f 66 74 5c 57 69 6e 64 6f 77 73 5c 43 75 72 72 65 6e 74 56 65 72 73 69 6f 6e 5c 55 6e 69 6e 73 74 61 6c 6c 5c } //75 SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\
		...
		$a_01_17 = {68 74 74 70 3a 2f 2f 35 73 74 61 72 76 69 64 65 6f 73 2e 63 6f 6d 2f 6d 61 69 6e 2f } //1100 http://5starvideos.com/main/
		$a_01_18 = {20 75 73 61 67 65 20 63 6f 75 6e 74 20 65 78 63 65 65 64 65 64 2c 20 70 6c 65 61 73 65 20 64 6f 77 6e 6c 6f 61 64 20 61 20 6e 65 77 20 76 65 72 73 69 6f 6e 2e } //75  usage count exceeded, please download a new version.
		$a_01_19 = {57 69 6e 64 6f 77 43 6c 61 73 73 } //50 WindowClass
		$a_01_20 = {20 69 6e 73 74 61 6c 6c 61 74 69 6f 6e 20 69 6e 66 6f 72 6d 61 74 69 6f 6e 20 77 61 73 20 63 6f 72 72 75 70 74 65 64 2c 20 70 6c 65 61 73 65 20 72 65 69 6e 73 74 61 6c 6c 20 } //75  installation information was corrupted, please reinstall 
	condition:
		((#a_00_0  & 1)*1+(#a_00_1  & 1)*2+(#a_00_2  & 1)*2+(#a_00_3  & 1)*1+(#a_00_4  & 1)*2+(#a_00_5  & 1)*2+(#a_01_6  & 1)*2+(#a_01_7  & 1)*1+(#a_00_8  & 1)*2+(#a_00_9  & 1)*2+(#a_01_10  & 1)*2+(#a_00_11  & 1)*1+(#a_00_12  & 1)*10+(#a_00_13  & 1)*10+(#a_00_14  & 1)*10+(#a_01_15  & 1)*75+(#a_01_16  & 1)*25+(#a_01_17  & 1)*1100+(#a_01_18  & 1)*75+(#a_01_19  & 1)*50+(#a_01_20  & 1)*75) >=1300
