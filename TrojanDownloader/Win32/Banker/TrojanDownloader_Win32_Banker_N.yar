...
rule TrojanDownloader_Win32_Banker_N{
	...
	strings :
		$a_03_0 = {8b 45 c0 b2 01 e8 ?? ?? ?? ?? 90 90 ff 35 ?? ?? ?? ?? ff 35 ?? ?? ?? ?? 68 ?? ?? ?? ?? ff 35 ?? ?? ?? ?? 8d 45 bc ba 04 00 00 00 e8 ?? ?? ?? ?? 8b 45 bc e8 ?? ?? ?? ?? 50 8d 45 b8 b9 50 04 43 38 8b 15 ec a0 43 38 e8 ?? ?? ?? ?? 8b 45 b8 } //2
		$a_03_1 = {85 d2 0f 84 c7 00 00 00 85 c9 0f 84 30 fb ff ff 3b 10 0f 84 be 00 00 00 3b 08 74 0e 50 51 e8 ?? ?? ?? ?? 5a 58 e9 } //2
