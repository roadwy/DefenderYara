...
rule TrojanDownloader_Win32_Banload_BFR{
	...
	strings :
		...
		$a_03_5 = {26 00 41 00 56 00 3d 00 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? [0-10] 76 00 69 00 73 00 75 00 61 00 6c 00 2e 00 70 00 68 00 70 00 } //1
		$a_03_6 = {61 70 70 64 61 74 61 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? [0-10] 4d 65 64 69 61 58 } //1
		$a_03_7 = {61 70 70 64 61 74 61 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? [0-10] 50 6c 75 67 69 6e 50 6c 61 79 65 72 } //1
	condition:
