...
rule TrojanDownloader_Win32_Sarhust_A{
	...
	strings :
		...
		$a_03_5 = {83 c4 0c 8d 8d ?? ff ff ff e8 ?? ?? ?? ?? 8d 45 ?? 50 6a 00 8d 85 ?? ff ff ff 50 68 ?? ?? ?? ?? 6a 00 6a 00 ff 15 ?? ?? ?? ?? 89 45 ?? 85 c0 74 } //1
		$a_03_6 = {55 8b ec 81 ec ?? ?? ?? ?? e8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 6a 00 e8 ?? ?? 00 00 83 c4 08 68 ?? ?? 00 00 ff 15 ?? ?? ?? ?? 8d 8d ?? ff ff ff e8 ?? ?? 00 00 8d 8d ?? ff ff ff e8 ?? ?? 00 00 8d 8d ?? ff ff ff e8 ?? ?? 00 00 6a } //1
	condition:
