...
rule TrojanDownloader_Win32_Delfhost_A{
	...
	strings :
		...
		$a_02_6 = {ff ff 84 c0 74 30 90 09 2a 00 50 e8 ?? ?? ?? ff 6a 01 a1 ?? ?? ?? 00 50 e8 ?? ?? ?? ff e8 ?? ?? ff ff eb 46 6a 06 a1 ?? ?? ?? 00 50 e8 ?? ?? ?? ff e8 } //1
		$a_02_7 = {ff ff 84 c0 74 30 90 09 25 00 50 e8 ?? ?? ?? ff 6a 01 a1 ?? ?? ?? 00 50 e8 ?? ?? ?? ff eb 46 6a 06 a1 ?? ?? ?? 00 50 e8 ?? ?? ?? ff e8 } //1
	condition:
