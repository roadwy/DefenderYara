...
rule TrojanDownloader_BAT_FormBook_ABO_MTB{
	...
	strings :
		$a_03_0 = {07 08 07 6f ?? ?? ?? 0a 1e 5b 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 07 17 6f ?? ?? ?? 0a 06 07 6f ?? ?? ?? 0a 17 73 ?? ?? ?? 0a 0d 16 2d 0e 16 2d 0b 09 02 16 02 8e 69 6f ?? ?? ?? 0a de 0d 1d 2c 03 09 2c 06 09 6f ?? ?? ?? 0a dc 06 6f ?? ?? ?? 0a 13 04 16 } //4
		$a_03_1 = {08 2b df 6f ?? ?? ?? 0a 2b da 08 2b dc 6f ?? ?? ?? 0a 2b d7 } //3
