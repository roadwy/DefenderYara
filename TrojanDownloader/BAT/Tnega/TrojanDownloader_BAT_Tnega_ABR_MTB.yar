...
rule TrojanDownloader_BAT_Tnega_ABR_MTB{
	...
	strings :
		$a_03_0 = {0c 07 08 07 6f ?? ?? ?? 0a 1e 5b 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 07 08 07 6f ?? ?? ?? 0a 1e 5b 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a 2b 07 6f ?? ?? ?? 0a 2b b0 07 17 6f ?? ?? ?? 0a 2b 07 6f ?? ?? ?? 0a 2b 98 06 07 6f ?? ?? ?? 0a 17 73 ?? ?? ?? 0a 0d 09 02 16 02 8e 69 6f ?? ?? ?? 0a de 0a 09 2c 06 09 6f ?? ?? ?? 0a dc 06 6f ?? ?? ?? 0a 13 04 de 14 } //5
