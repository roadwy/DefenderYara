...
rule TrojanDownloader_BAT_AgentTesla_ABO_MTB{
	...
	strings :
		$a_03_0 = {09 2c 07 09 6f 1a ?? ?? 0a 00 dc 08 6f 1b ?? ?? 0a 13 04 de 16 90 0a 49 00 00 72 01 ?? ?? 70 28 04 ?? ?? 06 0a 06 73 15 ?? ?? 0a 0b 00 73 16 ?? ?? 0a 0c 00 07 16 73 17 ?? ?? 0a 73 18 ?? ?? 0a 0d 00 09 08 6f 19 ?? ?? 0a 00 00 de 0b } //5
...
rule TrojanDownloader_BAT_AgentTesla_ABO_MTB_2{
	...
	strings :
		$a_03_0 = {0d 09 2c 68 00 06 72 ?? ?? ?? 70 08 72 74 ?? ?? 70 28 ?? ?? ?? 0a 6f ?? ?? ?? 0a 00 08 72 a0 ?? ?? 70 28 ?? ?? ?? 0a 28 ?? ?? ?? 0a 00 08 72 74 ?? ?? 70 28 ?? ?? ?? 0a 08 28 ?? ?? ?? 0a 00 1f 0a } //4
