...
rule TrojanDownloader_BAT_AgentTesla_ERZ_MTB{
	...
	strings :
		$a_03_0 = {0c 16 2d 12 08 06 6f ?? ?? ?? 0a 00 16 2d 07 06 6f ?? ?? ?? 0a 0d 09 13 04 17 2c f0 de 2c 02 2b d0 73 ?? ?? ?? 0a 2b cb 28 ?? ?? ?? 0a 2b c6 } //10
		$a_03_1 = {0c 08 06 6f ?? ?? ?? 0a 06 6f ?? ?? ?? 0a 0d 09 13 04 de 0a 90 09 17 00 03 73 ?? ?? ?? 0a 28 ?? ?? ?? 0a 0b 07 6f ?? ?? ?? 0a 6f ?? ?? ?? 0a } //10
