...
rule TrojanDownloader_O97M_Ocilo_A{
	...
	strings :
		$a_02_0 = {45 6e 76 69 72 6f 6e 28 22 54 45 4d 50 22 29 20 26 20 22 5c ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 2e 62 61 74 22 } //1
		...
		$a_02_2 = {43 61 6c 6c 20 53 68 65 6c 6c 28 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 2c 20 76 62 48 69 64 65 29 } //1
		$a_02_3 = {45 6e 76 69 72 6f 6e 28 22 54 45 4d 50 22 29 20 26 20 22 5c ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? 2e 74 78 74 22 } //1
	condition:
