...
rule TrojanDropper_Win32_Emold_E{
	...
	strings :
		$a_03_0 = {0f 31 0b c0 74 ?? 33 c0 64 8b 40 30 83 b8 b0 00 00 00 02 [0-06] 50 58 8b e4 6a 00 68 51 4f 68 57 54 90 13 b8 ?? ?? ?? ?? 81 c0 ?? ?? ?? ?? ff 55 } //1
	condition:
		((#a_03_0  & 1)*1) >=1
...
rule TrojanDropper_Win32_Emold_E_2{
	...
	strings :
		$a_03_0 = {0f 31 0b c0 74 [0-03] 33 c0 } //1
		...
		$a_03_2 = {6a 40 68 00 10 00 00 68 00 10 00 00 6a 00 b8 54 ca af 91 8b 75 14 ff d6 8b f8 eb 12 b9 ?? ?? ?? ?? f3 a4 5a 2b 55 14 89 45 14 03 d0 ff e2 e8 ?? ff ff ff } //1
	condition:
