...
rule TrojanProxy_Win32_Bedri_C{
	...
	strings :
		$a_03_0 = {ff ff 73 c6 85 ?? ?? ff ff 30 c6 85 ?? ?? ff ff 63 c6 85 ?? ?? ff ff 6b c6 85 ?? ?? ff ff 73 c6 85 ?? ?? ff ff 39 c6 85 ?? ?? ff ff 72 c6 85 ?? ?? ff ff 6f c6 85 ?? ?? ff ff 78 } //1
		$a_03_1 = {ff ff 62 c6 85 ?? ?? ff ff 38 c6 85 ?? ?? ff ff 65 c6 85 ?? ?? ff ff 64 c6 85 ?? ?? ff ff 72 c6 85 ?? ?? ff ff 69 c6 85 ?? ?? ff ff 33 c6 85 ?? ?? ff ff 68 c6 85 ?? ?? ff ff 38 c6 85 ?? ?? ff ff 6e c6 85 ?? ?? ff ff 62 } //1
...
rule TrojanProxy_Win32_Bedri_C_2{
	...
	strings :
		$a_03_0 = {b9 06 00 00 00 33 c0 8d bd ?? ?? ff ff f3 ab 66 ab 8d 85 ?? ?? ff ff 50 6a 00 68 03 00 1f 00 ff 15 90 09 0e 00 c6 85 ?? ?? ff ff 32 c6 85 ?? ?? ff ff 00 } //2
		$a_03_1 = {0f be 08 83 f9 2f 74 0b 8b 55 ?? 83 ea 01 89 55 ?? eb ea } //1
