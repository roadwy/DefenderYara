...
rule Backdoor_BAT_Crysan_ABH_MTB_2{
	...
	strings :
		$a_03_0 = {02 13 04 20 ?? ?? ?? 00 7e ?? ?? ?? 04 7b ?? ?? ?? 04 3a ?? ?? ?? ff 26 20 ?? ?? ?? 00 38 ?? ?? ?? ff 11 07 6f ?? ?? ?? 0a 11 04 16 11 04 8e 69 28 ?? ?? ?? 06 13 09 } //2
...
rule Backdoor_BAT_Crysan_ABH_MTB_3{
	...
	strings :
		$a_03_0 = {08 09 6f 19 ?? ?? 0a 7e 13 ?? ?? 04 73 1a ?? ?? 0a 13 06 11 06 02 7e 14 ?? ?? 04 02 8e 69 6f 1b ?? ?? 0a 11 06 6f 1c ?? ?? 0a dd 0f ?? ?? 00 11 06 39 07 ?? ?? 00 11 06 6f 1d ?? ?? 0a dc 08 6f 1e ?? ?? 0a 0a } //5
		$a_01_1 = {43 72 65 61 74 65 49 6e 73 74 61 6e 63 65 } //1 CreateInstance
