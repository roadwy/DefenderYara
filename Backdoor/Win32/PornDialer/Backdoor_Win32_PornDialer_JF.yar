...
rule Backdoor_Win32_PornDialer_JF{
	...
	strings :
		...
		$a_02_3 = {8b 8d 28 fe ff ff 6b c9 68 83 b9 ?? ?? ?? ?? 00 75 16 68 ?? ?? ?? ?? 8d 95 54 fc ff ff 52 e8 ?? ?? ?? ?? 83 c4 08 eb 2a 8b 85 28 fe ff ff 6b c0 68 83 b8 ?? ?? ?? ?? 01 75 16 68 ?? ?? ?? ?? 8d 8d 54 fc ff ff 51 e8 ?? ?? ?? ?? 83 c4 08 eb 02 eb 8d 68 c8 00 00 00 } //10
		$a_02_4 = {83 c4 08 85 c0 74 44 8d 95 30 f7 ff ff 52 8d 85 28 f5 ff ff 50 68 ?? ?? ?? ?? 68 08 02 00 00 8d 8d 28 f5 ff ff 51 e8 ?? ?? ?? ?? 83 c4 14 6a 00 8d 95 30 f7 ff ff 52 8d 85 28 f5 ff ff 50 68 ?? ?? ?? ?? e8 ?? ?? ?? ?? 83 c4 10 6a 10 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? e8 ?? ?? ?? ?? 83 c4 0c b9 ?? ?? ?? ?? e8 ?? ?? ?? ?? 6a 00 8b 0d ?? ?? ?? ?? 51 ff 15 ?? ?? ?? ?? 6a 00 6a 00 6a 12 8b 15 ?? ?? ?? ?? 52 ff 15 ?? ?? ?? ?? e9 ba 00 00 00 81 7d 10 eb 03 00 00 } //10
	condition:
