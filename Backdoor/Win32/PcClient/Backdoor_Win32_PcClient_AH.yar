...
rule Backdoor_Win32_PcClient_AH{
	...
	strings :
		...
		$a_03_2 = {6a 04 68 00 10 00 00 57 56 53 ff 15 ?? ?? ?? ?? 89 45 ?? 3b c6 74 ?? 56 57 ff 75 08 50 53 ff 15 ?? ?? ?? ?? 89 45 ?? 3b c6 74 ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? ff 15 ?? ?? ?? ?? 50 ff 15 ?? ?? ?? ?? 89 45 ?? 3b c6 74 ?? 56 56 ff 75 ?? 50 56 56 53 ff 15 ?? ?? ?? ?? 89 45 ?? 3b c6 74 ?? 6a ff 50 ff 15 } //1
	condition:
