...
rule Ransom_Win32_Gandcrab_J_MTB{
	...
	strings :
		...
		$a_02_1 = {ff ff 30 04 37 6a 00 ff 15 ?? ?? ?? 00 8d 85 ?? ?? ?? ?? 50 6a 00 ff 15 ?? ?? ?? 00 46 3b 75 08 7c db } //1
		$a_02_2 = {c1 e8 10 25 ff 7f 00 00 c3 90 0a 3f 00 a1 ?? ?? ?? 00 69 c0 ?? ?? ?? ?? 05 ?? ?? ?? ?? a3 ?? ?? ?? 00 c1 e8 10 25 ff 7f 00 00 c3 } //1
	condition:
