...
rule Ransom_Win32_Spora_B_{
	meta:
		description = "Ransom:Win32/Spora.B!!Spora.gen!rsm,SIGNATURE_TYPE_ARHSTR_EXT,08 00 08 00 0f 00 00 "
	...
	strings :
		...
		$a_03_8 = {83 7d 6c 04 0f 85 ?? ?? ?? ?? 57 8d 45 ?? 50 53 ff 15 ?? ?? ?? ?? 3b 45 ?? 0f 84 ?? ?? ?? ?? 39 5d ?? 75 07 8b 45 ?? 3b c6 72 05 89 75 ?? eb 06 83 e0 e0 } //1
		$a_03_9 = {68 10 66 00 00 ff 35 ?? ?? ?? ?? ff 15 ?? ?? ?? ?? 85 c0 0f 84 ?? ?? ?? ?? 8d 45 ?? 50 8d 45 ?? 50 53 6a 08 53 ff 75 ?? 89 7d ?? ff 15 ?? ?? ?? ?? 85 c0 0f 84 } //1
		$a_03_10 = {05 86 00 00 00 50 6a 40 ff 15 ?? ?? ?? ?? 8b f0 3b f3 0f 84 ?? ?? ?? ?? ff 35 ?? ?? ?? ?? ff 35 ?? ?? ?? ?? 56 ff 15 ?? ?? ?? ?? a1 ?? ?? ?? ?? 6a 02 } //2
		$a_03_11 = {f6 c3 01 74 ?? 6a 3a 8d 46 ?? 66 89 45 f0 58 ff 75 10 66 89 45 f2 ff 75 0c 33 c0 66 89 45 f4 8d 45 f0 50 ff 15 ?? ?? ?? ?? 50 8d 45 f0 50 ff 55 08 d1 eb 46 83 fe 1a 72 } //2
		$a_03_12 = {8d 77 14 f6 46 f8 02 74 0b 8d 46 ec 50 e8 ?? ?? ff ff eb ?? 83 7e f0 01 75 } //1
		$a_03_13 = {6a 06 56 ff 75 08 ff d7 8d 45 f8 50 56 56 53 ff 15 ?? ?? ?? ?? 50 53 ff 35 ?? ?? ?? ?? ff 15 } //1
		$a_03_14 = {68 00 02 00 00 ff 15 ?? ?? ?? ?? 46 83 fe 20 72 ?? eb 07 c7 45 fc 01 00 00 00 ff 75 ?? ff d7 53 ff d7 } //2
	condition:
