...
rule Ransom_Win32_Simlosap_A{
	...
	strings :
		$a_03_0 = {74 24 8d 45 ?? 33 d2 8a d3 83 c2 41 e8 ?? ?? ?? ?? 8d 45 ?? ba ?? ?? ?? ?? e8 ?? ?? ?? ?? 8b 45 ?? e8 ?? ?? ?? ?? 43 80 fb 1a 75 89 } //1
		$a_03_1 = {bb 05 00 00 00 e8 ?? ?? ?? ?? b8 1a 00 00 00 e8 ?? ?? ?? ?? 8b d0 80 c2 41 8d 85 ?? ?? ff ff e8 ?? ?? ?? ?? 8b 95 ?? ?? ff ff 8d 45 fc e8 ?? ?? ?? ?? 4b 75 d0 } //1
